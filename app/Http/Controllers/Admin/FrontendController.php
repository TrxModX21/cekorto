...
class FrontendController extends Controller
{
    ...
    public function frontendContent(Request $request, $key)
    {
        ...
        // CREATE VALIDATION MESSAGE FOR IMAGE INPUT AND SLUG
        foreach ($request->except('_token', 'video') as $inputField => $val) {
            ...
            if ($inputField == 'slug') {
                $validationRule[$inputField] = [
                    'required',
                    Rule::unique('frontends')->where(function ($query) use ($request) {
    ...
    public function remove($id)
    {
        ...
        $extractKeyType = explode('.', @$frontend->data_keys);
        $key = $extractKeyType[0];
        $type = $extractKeyType[1];
